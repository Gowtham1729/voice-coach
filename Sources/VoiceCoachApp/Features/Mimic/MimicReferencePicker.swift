...
struct MimicReferencePicker: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 14) {
      ...
      if let draft = model.mimicDraft {
        ...
        MimicTrimWaveform(
          peaks: draft.peaks, duration: draft.duration,
        ...
        if draft.duration > 30 {
          HStack(spacing: 12) {
            ...
            Text(
              "\(vcDuration(viewportStart))–\(vcDuration(viewportStart + 30)) of \(vcDuration(draft.duration))"
            )
        ...
        HStack(spacing: 16) {
          trimButtons(
            label: "Start", value: start, canDecrease: start > 0.001,
            canIncrease: start + 1.001 < end,
            decrease: { start = max(0, start - 0.1) },
            increase: { start = min(end - 1, start + 0.1) })
          Spacer()
          Text("\((end - start).formatted(.number.precision(.fractionLength(1)))) s selected")
          Spacer()
          trimButtons(
            label: "End", value: end, canDecrease: end - 1.001 > start,
            canIncrease: end + 0.001 < draft.duration,
            decrease: { end = max(start + 1, end - 0.1) },
            increase: { end = min(draft.duration, end + 0.1) })
        }
        .font(.caption.monospacedDigit())
        .foregroundStyle(Studio.secondary)
        ...
        HStack {
          Button(model.isPlaying ? "Stop Preview" : "Play Excerpt") {
            if model.isPlaying {
              ...
              model.playMimicDraft(start: start, end: end)
            }
          }
  ...
  private func trimButtons(
    label: String, value: Double, canDecrease: Bool, canIncrease: Bool,
    ...
    GeometryReader { geometry in
      ...
      ZStack(alignment: .leading) {
        Canvas { context, size in
          ...
          let last = min(
            peaks.count, Int(ceil(Double(peaks.count) * (viewportStart + visible) / duration)))
          ...
          for index in first..<last {
            ...
            let barWidth = max(
              1, size.width * CGFloat(duration / visible) / CGFloat(peaks.count) - 1)
            let bar = CGRect(x: x, y: (size.height - height) / 2, width: barWidth, height: height)
            context.fill(
              Path(roundedRect: bar, cornerRadius: 1),
              with: .color(Studio.accent.opacity(x >= left && x <= right ? 0.95 : 0.32)))
          }
        }
        Rectangle()
        ...
        DragGesture(minimumDistance: 0, coordinateSpace: .global).onChanged { value in
          ...
          if isStart {
            ...
            if endDragOrigin == nil { endDragOrigin = end }
              ...
              min(viewportStart + viewportDuration, (endDragOrigin ?? end) + change), start + 1)
          }
        }.onEnded { _ in
          if isStart { startDragOrigin = nil } else { endDragOrigin = nil }
        }
      )
      .accessibilityLabel(isStart ? "Excerpt start" : "Excerpt end")
      .accessibilityValue(
        "\((isStart ? start : end).formatted(.number.precision(.fractionLength(1)))) seconds"
      )
      .accessibilityAdjustableAction { direction in
        let delta = direction == .increment ? 0.1 : -0.1
        if isStart {
          start = min(max(0, start + delta), end - 1)
        } else {
          end = max(min(duration, end + delta), start + 1)
        }
      }
  }
}
