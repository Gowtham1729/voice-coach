...
struct MimicInspector: View {
    ...
    @ViewBuilder
    private func compareInspector(session: CoachingSession, reference: MimicReference, take: PracticeSession) -> some View {
        let takeNumber = (session.takes.firstIndex(where: { $0.id == take.id }) ?? 0) + 1
        let attemptStyle = session.mimicAttemptStyles?[take.id] ?? session.mimicStyle ?? .listenAndRepeat
        ...
        VStack(alignment: .leading, spacing: 14) {
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text("\(session.name) · \(attemptStyle.title) · \(vcNumber(metrics.duration, 1))s")
                    ...
                    .foregroundStyle(Studio.secondary)
                    .lineLimit(2)
                Text("Reference · \(reference.sourceName) · \(vcNumber(reference.take.result.metrics.duration, 1))s")
                    .font(.caption)
                    .foregroundStyle(Studio.secondary)
                    .lineLimit(2)
            }
            ...
            InspectorMetricCard(title: "Pitch Dynamic Range", value: vcOptional(metrics.pitchRangeSemitones), unit: "st", symbol: "waveform.path")
            InspectorMetricCard(title: "Trailing Energy Drop", value: vcSigned(metrics.phraseDecayDB), unit: "dB", symbol: "arrow.down.right")
            InspectorMetricCard(title: "Vocal Clarity (HNR)", value: vcOptional(metrics.hnrDB), unit: "dB", symbol: "sparkles")
            InspectorMetricCard(
                title: "Pause Cadence",
            ...
            VStack(spacing: 8) {
                ...
                .buttonStyle(.bordered)
                .controlSize(.regular)
                .help("Copy a mimic coach prompt with reference and take metrics. Includes transcripts; paste only where you trust.")
        ...
        VStack(alignment: .leading, spacing: 14) {
            VStack(alignment: .leading, spacing: 3) {
                ...
                Text(session.name)
                    .font(.headline)
                    .lineLimit(2)
                Text("Reference · \(reference.sourceName) · \(vcDuration(reference.take.result.metrics.duration))")
                    .font(.caption)
                    .foregroundStyle(Studio.secondary)
                    .lineLimit(2)
            }
