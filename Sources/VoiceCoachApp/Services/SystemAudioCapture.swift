...
final class SystemAudioCapture: SystemAudioCapturing, @unchecked Sendable {
  var onCaptureInterrupted: (() -> Void)?

  private let ioQueue = DispatchQueue(
    label: "com.gowtham.voicecoach.system-audio", qos: .userInteractive)
  ...
  private func handleIO(inputData: UnsafePointer<AudioBufferList>?, tapFormat: AVAudioFormat) {
    ...
    let snapshot = state.write { state -> (AVAudioFile, AVAudioFormat)? in
      ...
      guard state.isCapturing, !state.writeFailed,
        let file = state.audioFile, let format = state.writingFormat
      else { return nil }
      return (file, format)
    }
    ...
    guard
      let out = AVAudioPCMBuffer(pcmFormat: format, frameCapacity: AVAudioFrameCount(mono.count))
    else {
      return
    }
  ...
  private static func monoSamples(from buffer: AVAudioPCMBuffer, format: AVAudioFormat) -> (
    [Float], Float
  ) {
  ...
  private static func streamDescription(forTap tapID: AudioObjectID) throws
