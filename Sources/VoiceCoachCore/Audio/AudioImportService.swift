...
public struct AudioImportService {
  ...
  public static func prepareAudio(from sourceURL: URL, to destinationURL: URL) async throws {
    ...
    public static func trimAudio(
      from sourceURL: URL, to destinationURL: URL, start: Double, end: Double
    ) throws {
        ...
        if through > from {
          ...
          for channel in 0..<Int(input.processingFormat.channelCount) {
            destinationChannels[channel].update(
              from: sourceChannels[channel].advanced(by: Int(from)), count: count)
          }
    ...
    public static func waveform(from sourceURL: URL, buckets: Int = 0) throws -> (
      duration: Double, peaks: [Float]
    ) {
      let input = try AVAudioFile(forReading: sourceURL)
      ...
      while input.framePosition < input.length {
        ...
        for frame in stride(from: 0, to: Int(buffer.frameLength), by: 24) {
          let bucket = min(
            bucketCount - 1,
            Int(
              Double(start + AVAudioFramePosition(frame)) / Double(input.length)
                * Double(bucketCount)))
          for channel in 0..<Int(input.processingFormat.channelCount) {
