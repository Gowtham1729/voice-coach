...
public struct WordAcousticAnalyzer: Sendable {
    ...
    public func analyze(
        ...
        return transcription.words.map { word in
            ...
            let activeLoudnessPoints = loudnessPoints.filter { $0.value.isFinite && $0.value >= activeThreshold }
    ...
    private func startToEndMovement(
        ...
        guard validPitchFrames >= minimumPitchFramesStartToEnd,
              ...
              let lastTime = pitchPoints.last?.time else {
            return nil
        }
        ...
        guard earlyFrames.count >= minimumEdgeFramesStartToEnd,
              lateFrames.count >= minimumEdgeFramesStartToEnd else {
            return nil
        }
    ...
    private func loudnessMetrics(
        ...
        let activeFrameCoverage = totalFrames > 0 ? Double(activePoints.count) / Double(totalFrames) : 0.0
    ...
    private func loudnessMovement(activePoints: [TimePoint]) -> Double? {
        guard activePoints.count >= 4,
              ...
              let lastTime = activePoints.last?.time else {
            return nil
        }
